...
def ensure_swins_config(swint_repo):
    ...
    # FIX: Validate that depth substitution actually succeeded.
    # If the regex didn't match (e.g. due to YAML formatting variation),
    # the written config would silently be Swin-T depths, not Swin-S.
...
def main():
    ...
    cli_args = [
        ...
        # Explicitly override depths to Swin-S (2,2,18,2) in case config file differs
        ...
        # Set to 2500 to capture one mid-run checkpoint and one at the end
    ...
    # FIX: --resume must be appended at the end of cli_args.
    # Previously it was inserted at index 3, which placed it between
    # "--config-file" and its value, breaking argument parsing entirely.
    if args.resume:
