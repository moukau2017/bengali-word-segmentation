...
# ---------------------------------------------------------------------------
# Filter: keep only images where file exists on disk AND has annotations
# ---------------------------------------------------------------------------
 
def filter_valid_images(coco_json_path, images_dir):
    """
    Reads the COCO JSON and removes:
      - Images whose file is missing from images_dir
      - Images that have no annotations in the JSON
 
    Writes a filtered JSON next to the original and returns its path.
    """
    ...
    annotated_ids = set(a["image_id"] for a in all_anns)
    ...
    for img in all_images:
        file_on_disk = os.path.isfile(os.path.join(images_dir, img["file_name"]))
        has_ann      = img["id"] in annotated_ids
 
        if file_on_disk and has_ann:
            valid_images.append(img)
        else:
            if not file_on_disk:
                skipped_missing.append(img["file_name"])
            if not has_ann:
                skipped_no_ann.append(img["file_name"])
 
    valid_ids  = {img["id"] for img in valid_images}
    valid_anns = [a for a in all_anns if a["image_id"] in valid_ids]
    ...
    print(f"[filter] Filtered JSON saved to: {filtered_path}\n")
...
def split_coco_annotations(images_dir, coco_json_path, output_dir,
    ...
    print(f"[split] Total: {n}  →  train: {len(train_imgs)}  |  "
...
def main():
    ...
    if auto_split_mode:
        ...
        # ── Filter: only keep images with file on disk + annotation ──────
        filtered_json = filter_valid_images(args.json, args.images)
 
        train_json, val_json, _ = split_coco_annotations(
            images_dir=args.images, coco_json_path=filtered_json,
